...
* `service_account_path` - (Required, non-modifiable) The local path of the service_account JSON file for GCP authorization purposes. This service account is used to create the Connector in GCP. Either `service_account_path` or `service_account_key` must be set.  Conflicts with `service_account_key`.
